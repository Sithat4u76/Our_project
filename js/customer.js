const apiUrl = "https://script.google.com/macros/s/AKfycbz0A48OfPDoZTQ3AHqJZfaVsq-R-a21BFjr1047X2duCC7CWZwHBVtS1a0l6BjfYABK/exec";
  ...
  if (data.status === "success") {
    data.data.forEach(customer => {
      ...
      tr.innerHTML = `
        ...
        <td><img class="customer-img" src="${customer.image}" alt="customer Image" /></td>
        <td class="customer-actions">
          <button class="customer-edit customer-btn" onclick='editcustomer(${JSON.stringify(customer)})'>Edit</button>
          <button class="customer-delete customer-btn" onclick='deletecustomer("${customer.id}")'>Delete</button>
        </td>
      `;
...
document.getElementById("customer-addForm").addEventListener("submit", function (e) {
e.preventDefault();
...
document.getElementById("customer-editForm").addEventListener("submit", function (e) {
e.preventDefault();
...
if (data.status === "success") {
  ...
  alert("Failed to update customer: " + (data.message || "Unknown error"));
}
})
.catch(err => {
